...
use App\Bill\Domain\Client\Exceptions\CountryCodeValidationException;

class ZipValidator {

    private $countriesRules = [
        'AC' => '/^(?:ASCN 1ZZ)$/i',
        'AD' => '/^(?:AD[1-7]0\d)$/i',
        'AE' => null,
        'AF' => '/^(?:\d{4})$/i',
    ...
    public function check($zip, $countryCode)
    {
        if (!$this->validateCountryCode($countryCode)) {
            throw new CountryCodeValidationException($countryCode);
        }

        if (!$this->validateZipByCountryCode($zip, $countryCode)) {
    ...
    protected function validateCountryCode($countryCode)
    {
        ...
        if (!array_key_exists($countryCode, $this->countriesRules)) {
            return false;
        }
    ...
    protected function validateZipByCountryCode($zip, $countryCode)
    {
        return preg_match($this->countriesRules[$countryCode], $zip);
    }
}
